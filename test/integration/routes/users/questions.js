...
/*
    describe('PUT #/users/:username/questions/categories', function() {
      var categories = {
      ...
      it('should require authentication', function (done) {
        request(url)
          .put('/users/' + account.username + '/questions/categories')
          .set('Accept', 'application/json')
          .send(categories)
          .expect('Content-Type', 'application/json')
          .expect(401)
      ...
      it('should only let owning user account update the question categories', function(done) {
        request(url)
          .put('/users/' + account.username + Math.random() + '/questions/categories' )
          .set('Authorization', 'Bearer ' + account.oauth2.access_token)
          .set('Content-Type', 'application/json')
          .send(categories)
          .expect(401)
      ...
      it('should return 201 when questions saved', function (done) {
        request(url)
          .put('/users/' + account.username + '/questions/categories' )
          .set('Authorization', 'Bearer ' + account.oauth2.access_token)
          .set('Accept', 'application/json')
      ...
      it.skip('should create a questions object if one is not present', function() {

      });
    });

    describe('GET #/users/:username/questions/categories', function() {
      it('should require authentication', function (done) {
        request(url)
          .get('/users/' + account.username + '/questions/categories')
          .set('Accept', 'application/json')
          .expect('Content-Type', 'application/json')
          .expect(401)
          .end(done);
      ...
      it('should return an array of categories when questions found', function(done) {
        request(url)
          .get('/users/' + account.username + '/questions/categories')
          .set('Authorization', 'Bearer ' + account.oauth2.access_token)
          .set('Accept', 'application/json')
